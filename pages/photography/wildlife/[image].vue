<template>
  ...
  <header class="sticky top-0 bg-white shadow-md z-10">
    <div class="container mx-auto flex justify-between items-center px-4 py-3">
      <h1 class="text-2xl font-bold text-primary">{{ imageData.alt || 'Wildlife Photography' }}</h1>
      <button
          class="btn px-4 py-2 text-white bg-primary rounded hover:bg-primary-dark"
          @click="$router.push('/photography/wildlife')"
      >
      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-gray-50 p-6 rounded-lg shadow-md">
          ...
          <div class="space-y-3">
            ...
            <p><strong>Category:</strong> Wildlife</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="related-images-section py-16 bg-gray-50">
    <div class="container mx-auto px-4">
      <h2 class="text-3xl font-bold text-primary mb-8 text-center">More Wildlife Photography</h2>
      <div class="more-images-container">
        <image-gallery category="wildlife" :useDetailPages="false" class="fade-in"></image-gallery>
      </div>
    </div>
  </section>
...
const navigateToImage = (id) => {
  router.push(`/photography/wildlife/${id}`);
};

onMounted(async () => {
  const id = route.params.image;
  
  // Fetch all wildlife images to build navigation
  ...
  if (currentIndex > -1) {
    ...
    router.push('/photography/wildlife');
  }
  
  // Set SEO metadata
  useSeoMeta({
    title: `${imageData.value.alt || 'Wildlife Photography'} - Lasse Harm`,
    description: imageData.value.description || 'Detailed view of wildlife photography by Lasse Harm.',
