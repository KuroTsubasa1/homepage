<template>
  <div class="container mx-auto py-16 pt-24">
    <div class="text-center mb-8 animate-slide-up">
      <h1 class="text-3xl md:text-4xl font-bold text-white mb-4 gradient-text">{{ imageData.alt || 'Wildlife Photography' }}</h1>
      <NuxtLink to="/photography/wildlife" class="btn-neon-outline inline-block text-sm font-pixel">
        Back to Wildlife Gallery
      </NuxtLink>
    </div>

    <div class="max-w-4xl mx-auto px-4">
      <div class="cartridge overflow-hidden mb-8 animate-pixel-in">
        <img v-if="imageData.src" :src="imageData.src" :alt="imageData.alt" class="w-full" />
      </div>

      <div class="glass-card neon-border p-6 mb-8">
        <h2 class="text-xl font-semibold text-neon-cyan mb-4 font-pixel">Image Details</h2>
      ...
      <div class="flex justify-between items-center">
        <NuxtLink v-if="prevImage" :to="`/photography/wildlife/${prevImage.id}`" class="btn-neon-outline inline-block text-sm font-pixel">
          ← Previous
        </NuxtLink>
        <div v-else></div>
        <NuxtLink to="/photography/wildlife" class="btn-neon inline-block text-sm font-pixel animate-glow-pulse">Back to Gallery</NuxtLink>
        <NuxtLink v-if="nextImage" :to="`/photography/wildlife/${nextImage.id}`" class="btn-neon-outline inline-block text-sm font-pixel">
          Next →
        </NuxtLink>
        <div v-else></div>
      </div>
