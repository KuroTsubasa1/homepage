<template>
  ...
  <header class="sticky top-0 bg-white shadow-md z-10">
    <div class="container mx-auto flex justify-between items-center px-4 py-3">
      <h1 class="text-2xl font-bold text-primary">{{ imageData.alt || 'Abstract & Art Photography' }}</h1>
      <button
      ...
      <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-gray-50 p-6 rounded-lg shadow-md">
          ...
          <div class="space-y-3">
            ...
            <p><strong>Category:</strong> Abstract & Art</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="related-images-section py-16 bg-gray-50">
    <div class="container mx-auto px-4">
      <h2 class="text-3xl font-bold text-primary mb-8 text-center">More Abstract & Art Photography</h2>
...
const navigateToImage = (id) => {
  router.push(`/photography/abstract-art/${id}`);
};

onMounted(async () => {
  const id = route.params.image;
  
  // Fetch all abstract images to build navigation
  const allImagesResponse = await fetch(`https://pocket.lasseharm.space/api/collections/portfolio_images/records?filter=(category='abstract')&perPage=1000`);
  ...
  if (currentIndex > -1) {
    ...
    router.push('/photography/abstract-art');
  }
  
  // Set SEO metadata
  useSeoMeta({
    title: `${imageData.value.alt || 'Abstract & Art Photography'} - Lasse Harm`,
    description: imageData.value.description || 'Detailed view of abstract and artistic photography by Lasse Harm.',
